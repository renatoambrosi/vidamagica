...
const {
  buscarUsuarioPorTelefone, criarOuAtualizarUsuario, atualizarUsuario,
...
const { gerarAccessToken, autenticar, JWT_SECRET } = require('../middleware/autenticar');
...
async function enviarOTPWhatsApp(telefone, codigo, nome) {
  ...
  if (!gateway || !token) {
    console.warn('⚠️ Gateway não configurado — OTP não enviado');
    if (process.env.NODE_ENV !== 'production') console.log(`[DEV] OTP para ${telefone}: ${codigo}`);
    return false;
  }
  const saudacao = nome ? `Olá, ${nome.split(' ')[0]}!` : 'Olá!';
  const mensagem = `${saudacao} 🔐\n\nSeu código de acesso ao *Vida Mágica* é:\n\n*${codigo}*\n\nVálido por 10 minutos. Não compartilhe com ninguém.\n\n— Vida Mágica`;
  try {
    await axios.post(`${gateway}/enviar`, {
      telefone, mensagem,
      nome: nome || telefone,
      origem: 'auth-otp',
      imediato: true,
...
// POST /api/auth/solicitar-otp-email
// { email: "user@email.com" }  — requer autenticação (para vincular email)
router.post('/solicitar-otp-email', autenticar, async (req, res) => {
  try {
    ...
    const usuario = await require('../db').buscarUsuarioPorId(usuario_id);
    const codigo = gerarOTP();
...
// ── 3. VERIFICAR OTP + LOGIN ──────────────────────────────────────────────────
// POST /api/auth/verificar-otp
// { telefone, codigo, device_fingerprint? }
router.post('/verificar-otp', async (req, res) => {
  try {
    ...
    // Registra dispositivo (revoga anterior do mesmo tipo automaticamente)
    ...
    const primeiro_acesso = !usuario.nome || !usuario.email_verificado;
    let etapa_pendente = null;
    if (!usuario.nome) etapa_pendente = 'nome';
    else if (!usuario.email) etapa_pendente = 'email';
    else if (!usuario.email_verificado) etapa_pendente = 'verificar_email';

    console.log(`✅ Login: ${tel} | ${tipo} | ${nome_amigavel}`);
...
// POST /api/auth/verificar-otp-email
// { email, codigo }
...
router.post('/renovar', async (req, res) => {
  try {
    ...
    const access_token = gerarAccessToken({
      id: sessao.uid, telefone_formatado: sessao.telefone_formatado,
      plano: sessao.plano, nome: sessao.nome,
    });
    ...
    res.json({
      ...
      expires_in: 900,
      usuario: {
        id: sessao.uid, nome: sessao.nome, email: sessao.email,
        plano: sessao.plano, perfil_teste: sessao.perfil_teste,
        percentual_prosperidade: sessao.percentual_prosperidade,
        sementes: sessao.sementes, estagio_arvore: sessao.estagio_arvore,
      },
    });
  } catch (err) {
...
router.put('/perfil', autenticar, async (req, res) => {
  try {
    ...
    // Se mudou email, marca como não verificado
...
// { telefone, senha, device_fingerprint? }
...
// ── 12. ME (dados do usuário logado) ─────────────────────────────────────────
// GET /api/auth/me
router.get('/me', autenticar, async (req, res) => {
  try {
    const usuario = await require('../db').buscarUsuarioPorId(req.usuario.sub);
    if (!usuario) return res.status(404).json({ error: 'Usuário não encontrado' });
