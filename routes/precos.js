...
router.get('/precos', async (req, res) => {
  try {
    const result = await pool.query('SELECT key, dados FROM precos ORDER BY key');
    const precos = {};
...
// ── ADMIN: LER DADOS BRUTOS ──
router.get('/admin/precos', autenticar, async (req, res) => {
  try {
    const result = await pool.query('SELECT key, dados FROM precos ORDER BY key');
    const precos = {};
    result.rows.forEach(row => { precos[row.key] = row.dados; });
...
// ── ADMIN: SALVAR ──
router.post('/admin/precos', autenticar, async (req, res) => {
