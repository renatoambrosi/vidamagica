...
🧠 /teste
  ...
  // Seed automático de preços (idempotente — só insere chaves novas).
  // Garante que produtos novos cadastrados em routes/seed.js apareçam
  // automaticamente no painel admin sem necessidade de chamada manual.
  try {
    await seedPrecos();
  } catch (err) {
    console.error('⚠️  Falha no seed automático de preços (não-fatal):', err.message);
  }
});
